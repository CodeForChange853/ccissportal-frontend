...
const TicketCard = ({ ticket, onResolve, onReroute, resolving }) => {
    ...
    const accent = status === 'OPEN' ? 'var(--color-danger)' : 'var(--border-subtle)';

    return (
        <div style={{ background: 'var(--bg-depth)', border: `1px solid var(--border-subtle)`, borderLeft: `3px solid ${accent}`, borderRadius: 6, padding: '14px 16px', display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'start' }}>
            <div>
        ...
        >
            ...
            >
                ...
                <select value={department} onChange={e => setDepartment(e.target.value)} style={sel}>
                    ...
                    {DEPARTMENTS.map(d => <option key={d} value={d}>{DEPT_ICON[d]} {d}</option>)}
                </select>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    <button className="btn-ghost" onClick={onClose}>Cancel</button>
                    <button className="btn-primary" disabled={!department || loading} onClick={() => onConfirm(id, department)}>
...
const AdminSupport = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const ticketRes = await adminApi.fetchAllTickets();
    ...
    const visible = useMemo(() => {
        let list = tickets.filter(t => t.department === activeDept);
    ...
    const handleReroute = async (id, department) => {
        setRerouting(true);
        try {
            await adminApi.rerouteTicket(id, department);
            setTickets(prev => prev.map(t => (t.ticket_id ?? t.id) === id ? { ...t, department, ticket_status: 'RESOLVED', was_manually_rerouted: true } : t));
            toast.success(`Ticket #${id} re-routed to ${department}.`);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <PageHeader
                subtitle="Student-submitted tickets by department — resolve or re-route to the correct queue."
            />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                {[
                    ...
                    { label: 'PENDING', value: allPend, color: allPend > 0 ? 'var(--color-warning)' : 'var(--text-muted)' },
